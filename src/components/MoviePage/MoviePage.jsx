      ...
      <h1 className="mb-4">{movieInfo.title} {movieInfo.adult ? <small>18+</small> : null}</h1>
        ...
        <div className="col-lg-6">
          <ul className={PosterInfo}>
            ...
            {movieInfo.genres.length > 0 ? <li><span>Жанр:</span> {movieInfo.genres.map((genre, i) => <span> {genre.name}{movieInfo.genres.length - 1 !== i ? ',' : null} </span>)} </li> : null}
            ...
            {movieInfo.production_countries.length > 0 ? <li><span>Страна:</span> {movieInfo.production_countries.map(country => <span> {country.name} </span>)} </li> : null}
